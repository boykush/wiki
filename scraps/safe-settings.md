...
- 管理対象は GitHub API で扱う設定のみ（branch protection / labels / collaborators / teams / topics / custom properties / environments / rulesets 等）。**ファイル内容は扱えず** `.github/workflows/*.yml` の配布はできない（[[GitHub Actions]] の強制は ruleset / [[require workflows]] 経由）
