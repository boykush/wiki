#[[Programming]] #[[Continuous Integration]]

[[GitHub]]公式のコード品質機能。Code scanning を拡張し、Pull Request 上でコード品質の問題を検出して修正提案までを提供する。

- 2025 年に public preview として公開。GitHub Team / [[GitHub Enterprise Cloud|Enterprise Cloud]] の組織リポジトリのみ対応
- 指摘は PR 上にインライン表示され、[[GitHub Copilot]]によるワンクリック autofix を提示する
- スキャンは[[GitHub Actions]]上で実行され、Actions minutes を消費する
- [[適応度関数]]のコード品質カテゴリに該当する

<https://docs.github.com/en/code-security/concepts/code-quality/code-quality>
