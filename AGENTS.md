...
ingest 作業はリポジトリローカルの [.claude/skills/ingest/SKILL.md](.claude/skills/ingest/SKILL.md) のフローに従う（Claude Code では `/ingest` として呼び出せる。他エージェントは同ファイルを読んで手動で従う）。plugin の `scraps:ingest` は使わない（one-shot 生成の設計が対話ファースト運用と合わないため fork した）。要点: 対話で理解を確定してから書き出す二段階、本文は接続メインのアンカー構成、仕様詳細の転記禁止。Issue / RSS 経由の自動 intake はこのフローの対象外。
